...
public class November_06_11505 {
	...
	static class RMQ {
		...
		long init(ArrayList<Long> list, int left, int right, int node) {
			...
			return sum[node] = leftSum * rightSum % 1000000007;
		}
		long query(int left, int right, int node, int nodeLeft, int nodeRight) {
			...
			return query(left, right, node*2, nodeLeft, nodeMiddle) * query(left, right, node*2+1, nodeMiddle+1, nodeRight) % 1000000007;
		}
		long update(int index, long newValue, int node, int nodeLeft, int nodeRight) {
			...
			int nodeMiddle = (nodeLeft + nodeRight) / 2;
			return sum[node] = (update(index, newValue, node*2, nodeLeft, nodeMiddle) * update(index, newValue, node*2+1, nodeMiddle + 1, nodeRight) % 1000000007);
		}
		void print_sum() {
